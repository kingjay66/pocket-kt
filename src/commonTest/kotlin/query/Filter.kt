    ...
    fun filterByField2() = runBlocking {
        assertDoesNotFail {
            launch {
                ...
                sortedResponse.items.forEach { item -> println(item.field1);assertTrue(item.field2,"Item's retried from the request should have field2 = true") }
            }
            println()
        }
    }
    ...
    fun filterByBoth() = runBlocking {
        assertDoesNotFail {
            launch {
                ...
                sortedResponse.items.forEach { item -> println(item.field1);assertTrue(item.field1 > 3 && item.field2,"Item's retried from the request should have field2 = true and field1 greater than 3") }
            }
            println()
        }
    }
